...
const routes = function (config) {
   ...
   * @returns
   */
  exp.importCollectionBSON = async function (req, res) {
    if (!req.file) {
      return res.status(400).send('Missing file');
    }
    const { params: { collection: collectionName, database: dbName }, body: { totalChunks, currentChunk }, file } = req;
    ...
    const writeAsync = util.promisify(writer.write.bind(writer));
    for (let i = 0; i < totalChunks; i++) {
      const chunkPath = `./storage/${filename}.${i}`;
    ...
    writer.end();
    const mongorestore = spawn('./bin/mongorestore', [
      `--uri=${config.mongodb.connectionString}`,
      `--db=${dbName}`,
      `--collection=${collectionName}`,
      filepath,
    ]);
    mongorestore.stderr.pipe(res).on('close', () => {
