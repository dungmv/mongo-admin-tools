...
const CHUNK_SIZE = 100 * 1024 * 1024; // 1 MB chunk size
...
instance.interceptors.response.use(null, async (error) => {
  ...
  // Delay the retry by specified time
...
$(() => {
  ...
  document.querySelector('#import-bson').addEventListener('click', function (event) {
    event.preventDefault();

    const $target = document.querySelector('#import-bson-modal');
  ...
  document.querySelector('#import-bson-submit').addEventListener('click', async function (event) {
    /** @type {File} */
    const file = $('#import-bson-input-file')[0].files[0];
    const collection = $(event.currentTarget).attr('collection-name');

    $('#import-bson-log').addClass('d-none');
    $('#import-bson-spinner').addClass('d-none');
    const csrfToken = document.getElementsByName('_csrf')[0].value;
    const totalChunks = Math.ceil(file.size / CHUNK_SIZE);

    for (let currentChunk = 0; currentChunk < totalChunks; currentChunk++) {
      const data = new FormData();
      data.append('file', file.slice(currentChunk * CHUNK_SIZE, (currentChunk + 1) * CHUNK_SIZE), `${file.name}.${currentChunk}`);
      data.append('totalChunks', totalChunks);
      data.append('currentChunk', currentChunk + 1);
      // eslint-disable-next-line no-await-in-loop
      const res = await instance.post(`${ME_SETTINGS.baseHref}db/${ME_SETTINGS.dbName}/import-bson/${collection}`, data, {
        headers: {
        ...
        retryDelay: 2000, // Time between retries in ms
        onUploadProgress: (event) => {
          const percentCompleted = Math.round(100 * (event.loaded + currentChunk * CHUNK_SIZE) / file.size);
          $('#import-bson-progress').css('width', `${percentCompleted}%`);
          $('#import-bson-progress').text(`${percentCompleted}%`);
          if (event.loaded === event.total && (currentChunk + 1) === totalChunks) {
            $('#import-bson-progress').text('Done');
            $('#import-bson-spinner').removeClass('d-none');
          }
        },
      });
      if ((currentChunk + 1) === totalChunks) {
        $('#import-bson-log').text(res.data);
      }
    }

    $('#import-bson-spinner').addClass('d-none');
    $('#import-bson-log').removeClass('d-none');
  });
});
