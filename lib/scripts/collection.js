...
$(() => {
  ...
  document.querySelector('#import-bson-submit').addEventListener('click', function (event) {
    ...
    const csrfToken = document.getElementsByName('_csrf')[0].value;
    axios.post(`${ME_SETTINGS.baseHref}db/${ME_SETTINGS.dbName}/import-bson/${collection}`, data, {
      headers: {
        'Content-Type': 'multipart/form-data', 'X-CSRF-TOKEN': csrfToken,
      },
